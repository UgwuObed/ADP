...
class SalesService
{
    ...
    private function resolveStock(User $user, string $network, string $type): array
    {
        $credential = request()->get('_api_credential');

        if ($credential) {
            $stock = ApiCredentialStock::where('api_credential_id', $credential->id)
                ->where('network', $network)
                ->where('type', $type)
                ->first();

            return [$stock, true, $credential];
        }
        ...
        return [$stock, false, null];
    }

    private function lockStock(mixed $stock, bool $isCredentialStock): mixed
    {
        if ($isCredentialStock) {
            return ApiCredentialStock::where('id', $stock->id)->lockForUpdate()->first();
        }

        return DistributorStock::where('id', $stock->id)->lockForUpdate()->first();
    }
    ...
    public function sellAirtime(User $user, array $data): array
    {
        ...
        if ($amount < 50) {
            return ['success' => false, 'message' => 'Minimum airtime is ₦50'];
        }

        if ($amount > 50000) {
            return ['success' => false, 'message' => 'Maximum airtime is ₦50,000'];
        }

        [$stock, $isCredentialStock, $credential] = $this->resolveStock($user, $network, 'airtime');
        ...
        return DB::transaction(function () use (
            $user, $stock, $isCredentialStock, $phone, $amount, $network, $reference
        ) {
            $stock       = $this->lockStock($stock, $isCredentialStock);
            $stockBefore = $stock->balance;

            if ($stock->balance < $amount) {
                return ['success' => false, 'message' => 'Insufficient stock (concurrent transaction)'];
            }
            ...
            if ($apiResult['success']) {
                ...
                $this->notificationService->notifyAirtimeSale(
                    $user, $network, $phone, $amount, $reference
                );
    ...
    public function sellData(User $user, array $data): array
    {
        ...
        if (!$plan || !$plan->is_active) {
            return ['success' => false, 'message' => 'Data plan not found or unavailable'];
        }

        $network = strtolower($plan->network->code);
        $amount  = $plan->amount;

        [$stock, $isCredentialStock, $credential] = $this->resolveStock($user, $network, 'data');
        ...
        return DB::transaction(function () use (
            $user, $stock, $isCredentialStock, $phone, $plan, $network, $amount, $reference
        ) {
            $stock       = $this->lockStock($stock, $isCredentialStock);
            $stockBefore = $stock->balance;
